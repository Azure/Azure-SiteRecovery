...
if [ -f /etc/oracle-release ]; then
...
elif [ -f /etc/os-release ] && egrep -q 'SLES|openSUSE Leap' /etc/os-release; then
    ...
    elif grep -q 'DISTRIB_RELEASE=24.04' /etc/lsb-release ; then
	if [ `uname -m` = "x86_64" ]; then
            OS="UBUNTU-24.04-64"
        fi
    fi
    ...
    elif grep -q '^12.*' /etc/debian_version; then
        if [ `uname -m` = "x86_64" ]; then
            OS="DEBIAN12-64"
        fi
    fi
